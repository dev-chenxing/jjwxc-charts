...
const getNovelItem = (url) => {
    return new Promise(async (resolve, reject) => {
        ...
        axios.get(url, { responseType: "arraybuffer" }).then((response) => {
            ...
            console.log($("span[itemprop='collectedCount']")[0].next.next.next);
            console.log("parent: ")
            console.log($("span[itemprop='collectedCount']")[0].parent);
            // novel["credits"] = $("span[itemprop='collectedCount']")[0].next.next.next.text().trim().split('：').slice(-1)[0]
